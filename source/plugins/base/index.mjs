    ...
    try {
            ...
            for (const field in contribution) {
              if (indepthExtraFields.includes(field)) {
                for (const repo of contribution[field]) {
                  const {nameWithOwner} = repo.repository
                  if (!_owned && nameWithOwner.split("/")[0] === login)
                    continue
      ...
      for (const type of ({user: ["repositories", "repositoriesContributedTo"], organization: ["repositories"]}[account] ?? [])) {
        ...
        do {
          ...
          const request = {}
          const batch = Math.min(repositories, {user: _batch, organization: Math.min(25, _batch)}[account])
          try {
            Object.assign(request, await graphql(queries.base.repositories({login, account, type, after: cursor ? `after: "${cursor}"` : "", repositories: batch, ...options})))
          }
          catch (error) {
